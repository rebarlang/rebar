...
namespace Rebar.RebarTarget.LLVM
{
    internal class FunctionCompiler : VisitorTransformBase, IDfirNodeVisitor<bool>, IDfirStructureVisitor<bool>
    {
        ...
        static FunctionCompiler()
        {
            ...
            _functionalNodeCompilers["IsSome"] = CompileIsSome;
        ...
        private static void CompileIsSome(FunctionCompiler compiler, FunctionalNode isSomeNode)
        {
            // TODO: figure out how to generate calls to monomorphized functions for this
            ValueSource optionSource = compiler.GetTerminalValueSource(isSomeNode.InputTerminals[0]);
            LocalAllocationValueSource isSomeSource = (LocalAllocationValueSource)compiler.GetTerminalValueSource(isSomeNode.OutputTerminals[1]);
            LLVMValueRef optionPtr = optionSource.GetValue(compiler._builder),
                isSomePtr = compiler._builder.CreateStructGEP(optionPtr, 0u, "isSomePtr"),
                isSome = compiler._builder.CreateLoad(isSomePtr, "isSome");
            isSomeSource.UpdateValue(compiler._builder, isSome);
        }
